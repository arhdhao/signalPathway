 ...
 *                 反馈回路密集的通路尤其依赖它（见 设计资料/外部AI的建议与评价/ 里 Gemini 评审第二条）。
 ...
 * 关卡设计，所以它们在 nocgmp/game.ts 的 OUTCOME 里。
 *
 * 这里的数字以前是直接写在代码里的裸数字（所谓魔法数字），现在提上来并命名：
 * 好处是调引擎手感时只改这一处，而且看名字就知道它在管什么。
 * ⚠️ 改动这些值会影响【所有关卡】，改完要跑 tools/balance.mjs 与 tools/smoke-test.mjs。
 * ==========================================================================*/
export const TUNING = {
...
/** 关卡完整结构 —— nocgmp/index.ts 导出的 LEVEL 必须满足它 */
...
export class Simulator {
  ...
  tick() {
    ...
    // 不再有"遍历顺序导致的幽灵先手"（详见 设计资料/外部AI的建议与评价/ 里 Gemini 评审的第二条）。
 ...
 *   判胜阈值 / 红线 / 死法时长 / 结算文案 / 曲线    → pathways/nocgmp/
 * ==========================================================================*/
